...
namespace Text_classifier.Classification
{
    class SentenceLengthClassifier: IClassifier
    {
        ...
        private double CalculateDeviance(string text, double mean)
        {
            var sentences = Utils.ExtractSentences(text);
            double deviance = 0;
            foreach (var sentence in sentences)
                deviance += Math.Pow(Utils.ExtractWords(sentence).Count() - mean, 2);
            return deviance / (double)sentences.Count();
        }
